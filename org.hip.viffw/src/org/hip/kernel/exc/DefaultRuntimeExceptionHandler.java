...
/** A default implementation of the runtime exception handler.
 *
 * @author Benno Luthiger */

public class DefaultRuntimeExceptionHandler extends AbstractExceptionHandler {
    ...
    public Throwable convert(final Throwable inThrowableToBeConverted, final String inId) {
        final VThrowable outException = new VRuntimeException(inId);
    ...
    public static synchronized ExceptionHandler instance() { // NOPMD
        if (cInstance == null) {
            cInstance = new DefaultRuntimeExceptionHandler();
        }
        return cInstance;
    }
